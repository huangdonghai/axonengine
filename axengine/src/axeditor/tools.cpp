...
bool TransformTool::begin(bool mouse, int x, int y, int flags) {
	...
	m_beginMatrix = Matrix(m_gizmoAxis, m_gizmoCenter);
...
bool MoveTool::doTransform(const Vector3 &dist, int index, Matrix &result) {
#if 0
	bool v;
	Vector3 cliped, dist;
...
bool RotateTool::doTransform(const Vector3 &rotate, int index, Matrix &result ) {
	const AgentList &actorlist = m_context->getSelection();
...
bool ScaleTool::doTransform(const Vector3 &dist, int index, Matrix &result) {
