...
void GLdriver::initialize()
{
	...
	// init device info
	RenderDriverInfo *glDriverInfo = new RenderDriverInfo();
	glDriverInfo = glDriverInfo;
	glDriverInfo->driverType = RenderDriverInfo::OpenGL;
	...
	if (SUPPORT_GL_EXT_texture_compression_s3tc) {
		glDriverInfo->caps |= RenderDriverInfo::DXT;
	}
...
const RenderDriverInfo *GLdriver::getDriverInfo()
{
	return glDriverInfo;
}
...
HitRecords GLdriver::endSelect()
{
	...
	select_time = OsUtil::seconds() - select_time;
