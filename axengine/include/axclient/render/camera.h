...
namespace Axon { namespace Render {
	...
	public:
		...
		Convex() { m_numPlanes = 0; m_numVertexes = 0; m_numEdges = 0; }

		void initFromCamera(const Camera& camera);
		void initFromCamera(const Camera& shadowCamera, const Camera& visCamera);
		...
		int m_numVertexes;
		Vector3 m_vertexes[MAX_VERTEXES];
...
//------------------------------------------------------------------------------
	...
	public:
		...
		Camera createSelectionCamera(const Rect& region) const;
		Camera createMirrorCamera(const Plane& mirror_plane) const;
		...
		static void createCubemapCameras(const Camera& main, const AffineMat& mtx,  Camera result[6], float znear=0.5f, float zfar=16.0f);
		static void createCubemapCameras(const AffineMat& mtx,  Camera result[6], float znear=0.5f, float zfar=16.0f);
	...
	inline float Camera::getTop() const {
		return m_top;
	}
	...
	inline bool Camera::isOrthoProjection() const {
